...
class Message(db.Entity, BaseModel):
    ...
    def message_box1(self):
        return collipa.models.MessageBox.get(id=self.message_box1_id)

    @property
    def message_box2(self):
        return collipa.models.MessageBox.get(id=self.message_box2_id)

    @property
    def sender(self):
        return collipa.models.User[self.sender_id]

    @property
    def receiver(self):
        return collipa.models.User[self.receiver_id]
    ...
    def save(self):
        ...
        if not message_box1:
            message_box1 = collipa.models.MessageBox(sender_id=self.sender.id,
                                                     receiver_id=self.receiver.id,
                                                     status=1).save()
        if not message_box2:
            message_box2 = collipa.models.MessageBox(sender_id=self.receiver.id,
